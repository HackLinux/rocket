...
class Control extends Module
{
  ...
  val wb_set_sboard = wb_ctrl.div || wb_dcache_miss || wb_ctrl.rocc                       // used in hazard detection, indicating longer WB and update scoreboard 
  val replay_wb_common =                                                                  // dmem nack or RF/CSR replay
    io.dmem.resp.bits.nack || wb_reg_replay || io.dpath.csr_replay
  val wb_rocc_val = wb_reg_valid && wb_ctrl.rocc && !replay_wb_common
  val replay_wb = replay_wb_common || wb_reg_valid && wb_ctrl.rocc && !io.rocc.cmd.ready  // common wb replay or RoCC not ready
  ...
  io.imem.ras_update.bits.isReturn := mem_ctrl.jalr && io.dpath.mem_rs1_ra // valid function call as x1 used as LR
  ...
  bypassSrc(BYP_MEM) = (mem_reg_valid && mem_ctrl.wxd && !mem_ctrl.mem, io.dpath.mem_waddr) // bypass from WB when no memory load
  bypassSrc(BYP_DC) = (mem_reg_valid && mem_ctrl.wxd, io.dpath.mem_waddr)                   // bypass from WB with memory load
